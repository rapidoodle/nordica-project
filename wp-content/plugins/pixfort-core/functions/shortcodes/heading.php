...
if( ! function_exists( 'sc_heading' ) )
{
	function sc_heading( $attr, $content = null )
	{
        ...
        $output = '<div '.$heading_id.' class="'.$position.' '.$css_class.'">';
            ...
            $output .= '<div><div class="slide-in-container"><'.$title_tag.' class="'. $class_names .' el-title_custom_color mb-12" style="'.$t_custom_color. $t_size_style.'" data-anim-type="'. $animation .'" data-anim-delay="'.$delay.'">'. do_shortcode($title) .'</'.$title_tag.'></div></div>';
