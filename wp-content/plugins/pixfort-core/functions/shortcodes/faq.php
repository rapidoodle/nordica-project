...
if( ! function_exists( 'sc_pix_faq' ) )
{
	function sc_pix_faq( $attr, $content = null )
	{
        ...
        $output = '<div class="pix-faq '.$position.' '.$css_class.'">';
            ...
            $output .= '<div><div class="slide-in-container"><'.$title_tag.' class="d-flex align-items-center mb-3 animate-in" style="'.$t_custom_color. $t_size_style.'" data-anim-type="'. $animation .'" data-anim-delay="'.$delay.'">';
            ...
            if(!empty($content)){ $output .= '<div class="slide-in-container d-inline-block w-100"><div class="'.$c_classes.' '.$content_size.' animate-in" data-anim-type="'. $content_animation .'" data-anim-delay="'.$content_delay.'" style="'.$c_custom_color.'">'. do_shortcode( $content ) .'</div></div>'; }
