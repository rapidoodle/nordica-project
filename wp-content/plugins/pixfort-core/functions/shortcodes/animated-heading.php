...
if( ! function_exists( 'sc_animated_heading' ) )
{
	function sc_animated_heading( $attr, $content = null )
	{
		...
		$output = '<div class="text-'.$position.' '.$css_class.'">';
            ...
            if(!empty($slogan)){ $output .= '<div class="slide-in-container"><h6 data-anim-type="slide-in-up" class="'.$slogan_classes.' animate-in" '.$slogan_style.'>'. $slogan .'</h6></div>'; }
      		if(!empty($animation)){
				...
				$output .= '<div><'.$size.' class="pix-headline my-3 '.$display.' '.$style.' '.$t_color.' '.$title_bold.' '.$title_italic.' '.$title_secondary_font.'" style="'.$t_custom_color.'">';
      			...
      			<span class="pix-words-wrapper">'.$words_html.'
      			</span><span>'.$space_out.$br_out.do_shortcode( $text_after ).'</span>
      		</'.$size.'></div>';
            // if(!empty($content)){ $output .= '<div class="slide-in-container"><h5 data-anim-type="fade-in" class="text-gray-6 animate-in">'. do_shortcode( $content ) .'</h5></div>'; }
          $output .= '</div>';
