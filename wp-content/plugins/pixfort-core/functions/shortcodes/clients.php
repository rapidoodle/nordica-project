      ...
      'delay_items' 		=> '',
       ...
       $class_names = join( ' ', $classes );
        ...
        foreach ($clients_arr as $key => $value) {
            ...
            $output .= '<div class=" col-md col-md2 -'.$in_row.' '.$class_names.' text-center client2 d-inline-block2 d-inline-flex '.$style.'">';

            if( empty($value['title']) ){
                $value['title'] = '';
            }
            if( !empty($value['link']) ){
                $output .= '<a target="_blank" href="'. $value['link'] .'" class="client2 align-self-center py-3 d-inline-block w-100" title="'. $value['title'] .'">';
            }else{
                $title = '';
                if(!empty($value['title'])) {
                    $title = $value['title'];
            ...
            if( !empty($value['image']) ) {
                ...
                if(empty($imgSrcset)){
                    $output .= '<img src="'.$imgSrc.'" '.$imgWidth.' '.$imgHeight.' loading="lazy" class="animate-in" alt="'.$title.'" data-anim-type="'.$animation.'" data-anim-delay="'.$delay.'">';
                }else{
                    $output .= '<img src="'.$imgSrc.'" loading="lazy" srcset="'.$imgSrcset.'" sizes="'.$imgSizes.'" '.$imgWidth.' '.$imgHeight.' class="animate-in" alt="'.$title.'" data-anim-type="'.$animation.'" data-anim-delay="'.$delay.'">';
                }
